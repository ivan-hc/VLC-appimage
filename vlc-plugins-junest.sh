...
DEPENDENCES="ca-certificates ca-certificates-mozilla jre8-openjdk libaacs libbluray libbdplus libdvdcss libdvdnav libdvdread libvdpau libxtst libxi vlc-bittorrent zvbi $kvantumver" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="avc codec convert libGLX libsensors.so libsoxr libxcb svg video libSDL $lib_audio_keywords $lib_browser_launcher"
...
bind_dirs=" /media /mnt /opt /run/media /usr/lib/locale /usr/share/fonts /usr/share/Kvantum /usr/share/themes /var"
...
_remove_more_bloatwares() {
	...
	[ "$QTVER" = qt5-base ] && rm -R -f ./$APP.AppDir/.junest/usr/lib/qt6 ./$APP.AppDir/.junest/usr/lib/libQt6*
}
...
# Fix xcb error
rm -Rf ./"$APP".AppDir/.junest/usr/lib/qt* && rsync -av ./archlinux/.junest/usr/lib/qt* ./"$APP".AppDir/.junest/usr/lib/ || exit 1
...
# Fix vlc-cache-gen
rm -Rf ./"$APP".AppDir/.junest/usr/lib/vlc/* && rsync -av ./archlinux/.junest/usr/lib/vlc/* ./"$APP".AppDir/.junest/usr/lib/vlc/ || exit 1
...
REPO="VLC-appimage"
TAG="continuous-with-plugins"
VERSION="$VERSION"
UPINFO="gh-releases-zsync|$GITHUB_REPOSITORY_OWNER|$REPO|$TAG|*x86_64.AppImage.zsync"

ARCH=x86_64 ./appimagetool --comp zstd --mksquashfs-opt -Xcompression-level --mksquashfs-opt 20 \
	-u "$UPINFO" \
	./"$APP".AppDir "$APPNAME"_"$VERSION"-jre8-with-plugins-archimage4.3-x86_64.AppImage
